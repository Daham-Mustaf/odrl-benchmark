%--------------------------------------------------------------------------
% File     : ODRL043-1.p : TPTP v0.1.0
% Domain   : ODRL Policy Conflict Detection
% Problem  : OR-Conflict: spatial conflict ∨ purpose conflict → Conflict
% Expected : CounterSatisfiable (OR-Conflict)
% Verdict  : Conflict (disjunction — all operands conflict)
% Paper    : Definition 6 (Constraint Composition, disjunction)
%
% ODRL Scenario:
%   Permission:
%     { "constraint": [{
%         "leftOperand": "spatial",
%         "operator": "isPartOf",
%         "rightOperand": "westernEurope"
%       }],
%       "refinement": [{
%         "leftOperand": "purpose",
%         "operator": "isA",
%         "rightOperand": "dpv:AcademicResearch"
%       }] }
%   Prohibition:
%     { "constraint": [{
%         "leftOperand": "spatial",
%         "operator": "isPartOf",
%         "rightOperand": "easternEurope"
%       }],
%       "refinement": [{
%         "leftOperand": "purpose",
%         "operator": "isA",
%         "rightOperand": "dpv:Marketing"
%       }] }
%
%   Composition mode: OR
%
% Per-operand verdicts:
%   V_spatial:  isPartOf(wE) ∩ isPartOf(eE)                  [Conflict]
%   V_purpose:  isA(AcademicResearch) ∩ isA(Marketing)        [Conflict]
%   OR: ∀k: V_k = Conflict → verdict_or = Conflict
%
% Encoding: Disjunction — neither branch can produce a witness.
% Difficulty: Hard — both disjuncts must fail
% Authors  : Mustafa, D. & Sutcliffe, G.
%--------------------------------------------------------------------------
include('Axioms/Layer0-DomainKB/GEO000-0.ax').
include('Axioms/Layer0-DomainKB/DPV000-0.ax').
include('Axioms/Layer1-ODRLCore/ODRL000-0.ax').

fof(odrl043, conjecture,
    ( ?[Xs]: ( in_denotation(Xs, westernEurope, isPartOf)
             & in_denotation(Xs, easternEurope, isPartOf) )
    | ?[Xp]: ( in_denotation(Xp, academicResearch, isA)
             & in_denotation(Xp, marketing, isA) ) )).
%--------------------------------------------------------------------------
