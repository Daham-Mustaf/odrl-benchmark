...
fof(branch_a_purpose_def, axiom,
    ...
    has_value(c_branch_a_purpose, commercialPurpose)).
...
% For isNoneOf{commercialPurpose}, the if-direction (population rule):
% "If X is NOT under commercialPurpose, then X is in denotation"
fof(branch_a_purpose_if, axiom,
    ![X]: ((has_operand(c_branch_a_purpose, purpose) &
            has_operator(c_branch_a_purpose, isNoneOf) &
            has_value(c_branch_a_purpose, commercialPurpose) &
            taxonomic(purpose) &
            ~subClassOf(X, commercialPurpose))
        => in_denotation(X, c_branch_a_purpose))).

% The only-if direction (constraint on denotation):
fof(branch_a_purpose_only_if, axiom,
    ![X]: (in_denotation(X, c_branch_a_purpose)
          => ~subClassOf(X, commercialPurpose))).
