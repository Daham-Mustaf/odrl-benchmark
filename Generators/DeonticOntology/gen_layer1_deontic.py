...
=====================
Generates the Layer 1 deontic grounding axiom file:
  Problems/DeonticOntology/Axioms/Layer1-Deontic/GRND-AX-1.ax
...
% (FOL encoding of prose A1 in paper Section 4 / Appendix A.0)
%--------------------------------------------------------------------------
fof(ax_A1, axiom,
    ! [X, A, T, Q] :
...
%--------------------------------------------------------------------------
% B2  Power Content Links to Founding Event
% A Power with content decl(A) in relator Rho concerns the event
% that founded Rho.
%--------------------------------------------------------------------------
fof(ax_B2, axiom,
    ! [Pw, A, T, Rho, E, R] :
...
%--------------------------------------------------------------------------
% Summary:
%   Ax5.1-5.4  : 4 relator generation axioms (perm-basic, perm-strong,
%                  proh-basic, proh-remedy)
%   Ax5.5a-5.5b: 2 uniqueness axioms (founding, relator-per-event)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate GRND-AX-1.ax — Layer 1 deontic grounding axioms."
    )
    parser.add_argument(
        "--out-dir",
    ...
    # Write SMT-LIB axiom file (reference copy — SMT-LIB has no include)
    # Problem files embed these axioms directly; this file documents the shared theory.
    smt2_lines = [
        "; --------------------------------------------------------------------------",
        "; File     : GRND-AX-1.smt2",
        "; Domain   : Deontic Ontology / ODRL Grounding",
