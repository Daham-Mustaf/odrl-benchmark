...
Covers axioms not yet tested in problem_data.py:
  ...
  GRND011  Ax5.6  — obl creates Duty + Claim
  GRND012  Ax5.7  — correlativity duty <-> claim
  GRND013  Ax5.7  — correlativity power <-> subjection
  GRND014  Ax5.7  — correlativity immunity <-> disability
  GRND015  Ax5.5a — unique founding
  GRND016  Ax5.8  — conflict detection within relator
...
(assert (not
  (exists ((im Position) (db Position))
    (and (immunity im)   (bearer im alice) (cnt im read d1) (part-of im rho1)
         (disability db) (bearer db acme)  (cnt db read d1) (part-of db rho1)))))""",
    },

    # =========================================================================
    # GRND011 — Ax5.6: Obligation creates Duty + Claim
    # Difficulty: Easy
    # =========================================================================
    {
        "id": "GRND011-obl-relator", "subdir": "Entailment",
        "name": "Obligation creates Duty and Claim",
        "status_fof": "Theorem",
        "status_smt": "unsat",
        "fof_axioms": ["ax_obl_relator"],
        "description": """\
% obl(d1) activated by e1 entails Duty(alice,read,d1) and Claim(acme,read,d1).""",
        "ttl": """\
@prefix odrl:   <http://www.w3.org/ns/odrl/2/> .
@prefix drk:    <http://w3id.org/drk/ontology/> .
@prefix dcat:   <http://www.w3.org/ns/dcat#> .
...
fof(action_read, axiom, action(read)).
fof(target_d1,   axiom, target(d1)).
fof(rule_d1,     axiom, rule(d1)).
fof(event_e1,    axiom, event(e1)).
fof(obl_d1,      axiom, obl(d1)).
fof(aee_d1,      axiom, aee(d1, alice)).
fof(aer_d1,      axiom, aer(d1, acme)).
fof(act_d1,      axiom, act(d1, read)).
fof(tgt_d1,      axiom, tgt(d1, d1t)).
fof(target_d1t,  axiom, target(d1t)).
fof(act_e1_d1,   axiom, activates(e1, d1)).
""",
        "fof_conjecture": """\
? [Rho, Du, C] :
  ( founds(e1, Rho, d1)
  & duty(Du)  & bearer(Du, alice) & cnt(Du, read, d1t) & part_of(Du, Rho)
  & claim(C)  & bearer(C,  acme)  & cnt(C,  read, d1t) & part_of(C,  Rho) )""",
        "smt2_extra_decls": """\
(declare-const alice  Agent) (declare-const acme  Agent)
(declare-const read   Action) (declare-const d1t  Target)
...
(assert (not
  (exists ((rho Relator) (du Position) (c Position))
    (and (founds e1 rho obl1)
         ...
         (claim c) (bearer c acme)   (cnt c  read d1t) (part-of c  rho)))))""",
    },
    ...
    # =========================================================================
    {
        ...
        "name": "Correlativity: Duty implies unique Claim in relator",
        "status_fof": "Theorem",
        "status_smt": "unsat",
...
(assert (not (= rho1 rho2)))""",
    ...
    # =========================================================================
    # GRND016 — Ax5.8: Conflict detection within a single relator
    # Difficulty: Easy
    # =========================================================================
    {
        "id": "GRND016-conflict-relator", "subdir": "Entailment",
        "name": "Conflict detection: Liberty and Duty in same relator",
