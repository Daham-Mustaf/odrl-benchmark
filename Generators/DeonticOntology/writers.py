...
# Import preamble from gen_signature so it never diverges
...
def write_fof_problem(p: dict, out_dir: Path) -> Path:
    ...
    # TTL summary in header (comment form, no % prefix on content lines)
    if p.get("ttl"):
        lines.append("%")
        lines.append("% ODRL Policy (Turtle) — see Policies/ for full file:")
        for ttl_line in p["ttl"].strip().splitlines():
            # blank lines and comment lines stay as-is with % prefix
            if ttl_line.startswith("#") or ttl_line == "":
